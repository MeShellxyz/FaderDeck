...
AppHost::AppHost(const AppConfig &config)
    : m_appConfig(config),
      m_mixerSharedState(m_appConfig.audioConfig.num_channels,
                         m_appConfig.audioConfig.mute_buttons),
      m_serialListener(m_isRunning, m_appConfig.serialConfig,
                       m_mixerSharedState),
...
void AppHost::stop() {
    ...
    m_mixerSharedState.version.fetch_add(1, std::memory_order_acq_rel);
    m_mixerSharedState.version.notify_all();

    m_serialListener.stop();
}